...
# Load HumanizeDataset at startup
# Default dataset path - can be overridden via environment variable
DATASET_PATH = Path("datasets/humanize.parquet")
_dataset: HumanizeDataset | None = None


@app.on_event("startup")
async def load_dataset():
    """Load HumanizeDataset on application startup."""
    global _dataset
    try:
        _dataset = HumanizeDataset.load(str(DATASET_PATH))
    except FileNotFoundError:
        # Dataset file not found - lexical simplification will be unavailable
        _dataset = None
    except ValueError as e:
        # Invalid dataset file - log error but don't crash
        print(f"Warning: Failed to load dataset: {e}", file=sys.stderr)
        _dataset = None


def get_dataset() -> HumanizeDataset:
    """Get the loaded HumanizeDataset, raising an error if not available."""
    if _dataset is None:
        raise HTTPException(
            status_code=503,
            detail=f"Dataset not available. Please ensure {DATASET_PATH} exists and is valid.",
        )
    return _dataset
...
<body>
    <div class="container">
        ...
        <form id="humanize-form">
            ...
            <div class="options-container" style="margin-bottom: 24px;">
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                    <input type="checkbox" id="lexical-checkbox" style="width: 18px; height: 18px; cursor: pointer;">
                    <span>Enable lexical simplification (reduces vocabulary complexity)</span>
                </label>
            </div>
    ...
    <script>
        ...
        const lexicalCheckbox = document.getElementById('lexical-checkbox');
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('/api/humanize', {
                    ...
                    body: JSON.stringify({ 
                        text: text,
                        lexical: lexicalCheckbox.checked
                    }),
                });
...
async def humanize_text(request: TextRequest):
    ...
    try:
        ...
        # Apply lexical simplification if requested
        if request.lexical:
            dataset = get_dataset()
            simplifier = LexicalSimplifier(dataset)
            humanized = simplifier.simplify(humanized)
